...
## Diagram Generation with Natural Language

The BPM framework includes powerful diagram generation capabilities using ProcessPiper.

### Quick Start

```python
from bpm import StateMachine, StateDefinition, DiagramGenerator, render_diagram

# Define your process
machine = StateMachine()
machine.add_state(StateDefinition(name="draft", description="Draft"))
machine.add_state(StateDefinition(name="approved", description="Approved", is_terminal=True))
...
# Render to PNG/SVG/BPMN
render_diagram(piperflow_syntax, "my_process.png", export_bpmn=True)
```

### Natural Language PiperFlow Syntax

You can also write diagrams directly in natural language:
...
pool: Purchasing Department
    lane: Requester
        ...
        [Enter Details] as enter_details
...
start->create_req->enter_details->review->decision
decision->sign: Yes
decision->end: No
sign->end
...
PiperFlow supports various BPMN elements using simple syntax:

| Element Type | Syntax | Example |
|--------------|--------|---------|
| Start Event | `(start)` | `(start) as start` |
| End Event | `(end)` | `(end) as end` |
| Task | `[Name]` | `[Process Order] as process` |
| Subprocess | `[@subprocess Name]` | `[@subprocess Approval] as approval` |
| Gateway | `<Question?>` | `<Approved?> as decision` |
| Parallel Gateway | `<@parallel Name>` | `<@parallel Split> as split` |
| Timer Event | `(@timer Name)` | `(@timer Daily) as timer` |
| Message Event | `(@message Name)` | `(@message Notify) as notify` |

### Connection Labels

Add labels to connections to describe transitions:

```
state1->state2: Approved
state1->state3: Rejected
```

### Diagram Demo

Run the comprehensive diagram generation demo:

```bash
python examples/distributed_orchestration/bpm_examples/diagram_generation_demo.py
```

This demo shows:
- Automatic diagram generation from state machines
- Natural language PiperFlow syntax
- Pools and lanes
- Interactive diagram building
- Export to PNG/SVG/BPMN

### Installation

To use diagram generation:
...
- **Diagram Generation Demo**: `examples/distributed_orchestration/bpm_examples/diagram_generation_demo.py`
...
- **ProcessPiper Documentation**: https://github.com/csgoh/processpiper
